...
export function AudioDrawer({ opened, onClose, audio, initial_files, on_submit, is_loading }: Props) {
  ...
  useEffect(() => {
    if (!opened) {
      ...
      setFiles([{
        ...
        progress: 0,
        status: 'idle',
        categories: audio.categories?.map(c => c.id) || []
      }]);
  ...
  return (
    <Drawer
      opened={opened}
      onClose={onClose}
      title={audio ? t('edit_audio') : t('add_audio')}
      position="right"
      size="md"
      classNames={{
        header: 'px-6 py-4',
        ...
        {!audio && (
          ...
          >
            <Group justify="center" gap="xl" mih={120} style={{ pointerEvents: 'none' }}>
              ...
              <Dropzone.Idle>
                <IoMusicalNotesOutline size={50} color="var(--mantine-primary-color-filled)" />
              </Dropzone.Idle>
          ...
          {files.map((fileObj) => (
            <Paper key={fileObj.id} withBorder p="sm" radius="md" className="bg-white/5 border-white/10">
              <Stack gap="xs">
                ...
                {/* Audio Preview */}
                {(fileObj.file || (audio && fileObj.id === audio.id)) && (
                   <AudioPlayer 
                    src={fileObj.file ? URL.createObjectURL(fileObj.file) : (audio?.file_url || '')} 
                    class_name="mt-1"
                   />
                )}
