...
export const useAudios = (filters: any = {}) => {
  const t = useTranslations('Materials.audio');
  const common_t = useTranslations('Common');
  const query_client = useQueryClient();

  const query = useQuery({
    queryKey: queryKeys.materials.audio.list(filters),
    queryFn: () => audioActions.get_audios(filters),
  });

  const upload_mutation = useMutation({
    mutationFn: (data: { name: string; file: File; categories?: string[]; onProgress?: (p: any) => void }) => 
      audioActions.create_audio({ name: data.name, file: data.file, categories: data.categories }, data.onProgress),
    onSuccess: () => {
  ...
  const update_mutation = useMutation({
    mutationFn: (data: { id: string; name?: string; file?: File; categories?: string[]; onProgress?: (p: any) => void }) => 
      audioActions.update_audio(data.id, { name: data.name, file: data.file, categories: data.categories }, data.onProgress),
    onSuccess: () => {
      query_client.invalidateQueries({ queryKey: queryKeys.materials.audio.all() });
    },
    onError: (error: any) => {
  ...
  const delete_mutation = useMutation({
    mutationFn: audioActions.delete_audio,
    onSuccess: () => {
      query_client.invalidateQueries({ queryKey: queryKeys.materials.audio.all() });
      notifications.show({
  ...
  const bulk_delete_mutation = useMutation({
    mutationFn: audioActions.bulk_delete_audios,
    onSuccess: () => {
      query_client.invalidateQueries({ queryKey: queryKeys.materials.audio.all() });
      notifications.show({
        title: common_t('success'),
  ...
  return {
    ...
    upload_audio: upload_mutation.mutateAsync,
    update_audio: update_mutation.mutateAsync,
    delete_audio: delete_mutation.mutateAsync,
